...
const bot = ref({
  id: 2,
  name: chatStore.chatHistory[0].user.name,
...
const sendChatMessage = async (content: string = userMessage.value) => {
  ...
  */
  const body: ai00Type.OaiChatCompletionsType = {
    ...
    stop: ["\n\n","\nUser:","User:"],
    stream: true,
    names:{user:"主人",assistant:chatStore.chatHistory[0].user.name}
  }
...
<template>
  <v-textarea v-model="userMessage" rows="3" row-height="12" :label="$t('chat.chating')" no-resize max-rows="3"
      variant="outlined" clear-icon="mdi-close-circle" clearable :disabled="ischat" @keydown.ctrl.enter="sendMessage">
    <template #append>
      <v-btn style="margin-right: 20px; height: 70px" class="bg-primary" @click="sendMessage">
